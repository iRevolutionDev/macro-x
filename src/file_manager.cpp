...
{
    m_base_dir = base_dir;
    ensure_folder_exists(m_base_dir);
...
file file_manager::get_project_file(const std::filesystem::path& file_path) const
{
    ...
    return ensure_file_can_be_created(m_base_dir / file_path);
}

folder file_manager::get_project_folder(const std::filesystem::path& folder_path) const
{
